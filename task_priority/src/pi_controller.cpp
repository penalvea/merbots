...
void PIController::updateController(std::vector<float> odom, std::vector<float> current_joints, ros::Time time){
  ...
  if(init_){
    ...
    i_accum_[0]+=last_vels_[0]-(lvMcv.translation()[0]/seconds);
    i_accum_[1]+=last_vels_[1]-(lvMcv.translation()[1]/seconds);
    i_accum_[2]+=last_vels_[2]-(lvMcv.translation()[2]/seconds);
    i_accum_[3]+=last_vels_[3]-(rot_euler[2]/seconds);
    ...
    for(int i=4; i<last_vels_.size(); i++){
      ...
      i_accum_[i]=last_vels_[i]-current_vel;
    }
  }
  //std::cout<<i_accum_.size()<<std::endl;
  for(int i=0; i<i_accum_.size(); i++){
...
Eigen::MatrixXd PIController::getVels(Eigen::MatrixXd vels, ros::Time time){
  for(int i=0; i<last_vels_.size(); i++){
    ...
    vels(i,0)=(p_values_[i]*vels(i,0))+(i_values_[i]*i_accum_[i]);
  }
  last_time_=time;
  return vels;
}
