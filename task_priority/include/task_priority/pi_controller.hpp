...
class PIController{
  std::vector<float> p_values_, i_values_, d_values_;
  std::vector<float> error_accum_;
  std::vector<float> error_derivative_;
  std::vector<float> error_;
  std::vector<float> last_vels_, last_joints_, last_odom_, last_error_;
  Eigen::Transform<float, 3, Eigen::Affine> wMlv_;
  ros::Time last_time_;
  bool init_;

public:
  PIController(std::vector<float> p_values, std::vector<float> i_values, std::vector<float> d_values);
  ~PIController();
